...
export async function getAllPolls(req:AuthenticatedRequest, res:Response) {
    try {
        const result = await poll_service.getAllPolls();
...
export async function postPoll(req:AuthenticatedRequest, res:Response) {
    ...
    try {
        const result = await poll_service.createPoll({...params, user_id});
